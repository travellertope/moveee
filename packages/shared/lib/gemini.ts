...
const SYSTEM_PROMPT = `You are a knowledgeable curator for The Moveee's Culture Directory — a wiki-like reference celebrating culture.
...
export async function generateTopicSuggestions(
  ...
  const prompt = `You are a curator for The Moveee's Culture Directory — a growing wiki celebrating culture.
 ...
 * thinkers, artists, and leaders across every culture and region.
 *
...
export async function searchAndExtractQuotes(
  ...
  const prompt = `You are a quote verification assistant for The Moveee — a culture platform.
...
export async function verifyExistingQuote(
  ...
  const prompt = `You are a quote fact-checker for The Moveee — a culture platform. Your job is to assess whether a quote in our database is authentic.
...
export async function enrichEventContent(
  ...
  const prompt = `You are the editorial writer for The Moveee — a cultural platform for people who live for culture. Use Google Search to research this event and write a rich editorial description for its detail page.
...
1. What the event is, who's involved, and why it matters to the local cultural community
...
export async function evaluateAndExtractEvents(
  ...
  const prompt = `You are the events curator for The Moveee — an independent cultural platform for people who live for culture. Today's date is ${currentDate}.
